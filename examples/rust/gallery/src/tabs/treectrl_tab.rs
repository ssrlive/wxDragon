...
impl TreeCtrlTabControls {
    pub fn bind_events(&self) {
        ...
        self.tree_ctrl.bind(EventType::TREE_SEL_CHANGED, move |event| {
        ...
        self.tree_ctrl.bind(EventType::TREE_ITEM_ACTIVATED, move |event| {
            if let Some(item_id) = event.get_item() {
                if let Some(item_data) = tree_ctrl.get_item_data(&item_id) {
                    info_text.set_label(&format!(
...
pub fn create_treectrl_tab(parent: &Notebook) -> TreeCtrlTabControls {
    ...
    let root_id = tree_ctrl.add_root_with_data("Company Hierarchy", ceo_data).unwrap();
    ...
    let eng_id = tree_ctrl.append_item_with_data(&root_id, "Engineering", eng_project).unwrap();
    ...
    tree_ctrl.append_item_with_data(&eng_id, "Alice Johnson", eng_lead).unwrap();
    ...
    tree_ctrl.append_item_with_data(&eng_id, "Bob Williams", dev1).unwrap();
    ...
    tree_ctrl.append_item_with_data(&eng_id, "Carol Davis", dev2).unwrap();
    
    // Marketing department with String data
    let marketing_id = tree_ctrl.append_item_with_data(
        &root_id, 
        "Marketing", 
    ...
    tree_ctrl.append_item_with_data(&marketing_id, "David Wilson", 
        PersonData {
    ...
    tree_ctrl.append_item_with_data(&marketing_id, "Current Campaign", 
        ProjectData {
    ...
    let finance_id = tree_ctrl.append_item_with_data(&root_id, "Finance", 250).unwrap();
    ...
    tree_ctrl.append_item_with_data(&finance_id, "Eve Brown", finance_lead).unwrap();
