...
impl FontPickerCtrl {
    ...
    pub fn get_selected_font(&self) -> Option<Font> {
        unsafe {
            let font_ptr = ffi::wxd_FontPickerCtrl_GetSelectedFont(self.window.as_ptr() as *mut ffi::wxd_FontPickerCtrl_t);
            if font_ptr.is_null() {
                None
            } else {
...
impl<'a> FontPickerCtrlBuilder<'a> {
    ...
    pub fn build(self) -> FontPickerCtrl {
        ...
        let initial_font_ptr = self.initial_font.as_ref().map_or(ptr::null(), |f| f.as_ptr());
